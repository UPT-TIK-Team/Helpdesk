$(document).ready(function () {
  ...
  setInterval(function () {
    $(".rel-time").each(function (elem) {
      ...
      if (value)
        $(this).html(
          '<span data-toggle="tooltip" data-placement="bottom"  data-html="true" title="" data-original-title="' +
            getDateTime(parseInt(value)) +
            '">' +
            getRelativeTime($(this).attr("data-value")) +
            "</span>"
        );
      else $(this).html("-");
    });
...
export function renderCustomHTML() {
  ...
  $(".tik-severity").each(function (elem) {
    var value = $(this).attr("data-value");
    if (value) $(this).html(getSeverityIcon($(this).attr("data-value")));
    else $(this).html("-");
  });

  $(".tik-status").each(function (elem) {
    var value = $(this).attr("data-value");
    if (value) $(this).html(getStatusIcon($(this).attr("data-value")));
    else $(this).html("-");
  });

  $(".tik-category").each(function (elem) {
    var value = $(this).attr("data-value");
    if (value) $(this).html(getCategoryIcon($(this).attr("data-value")));
    else $(this).html("-");
  });
  ...
  $(".activity-icon").each(function (elem) {
    ...
    if (value) $(this).addClass(getActivityIconClass(value));
  });
...
function getStringInitials(str) {
  ...
  var acr =
    matches.length > 1
      ? [matches[0], matches[matches.length - 1]].join("")
      : matches.join("");
...
function getUserIcon(name, username) {
  if (username)
    return (
      '<div class="userIcon" style="background:#' +
      intToRGB(hashCode(username)) +
...
export function getUserLabel(name, username) {
...
function getUserTypeLabel(type) {
  var type_map = {
    10: '<div class="badge badge-corner bg-blue">User</div>',
    60: '<div class="badge badge-corner bg-green">Agent</div>',
    80: '<div class="badge badge-corner bg-orange">Manager</div>',
    100: '<div class="badge badge-corner bg-red">Admin</div>',
  };
...
function getUserStatus(status) {
  var status_map = {
    0: '<div class="badge badge-corner bg-grey">Disabled</div>',
    1: '<div class="badge badge-corner bg-green">Active</div>',
  };
...
function getSeverityIcon(severity) {
  var severity_map = {
    0: '<div class="badge badge-corner bg-green">Low</div>',
    5: '<div class="badge badge-corner bg-yellow">Medium</div>',
    10: '<div class="badge badge-corner bg-red">High</div>',
  };
...
function getStatusIcon(priority) {
  var priority_map = {
    ...
    50: '<div class="badge badge-corner bg-yellow">In-Progress</div>',
...
export function getAttachmentLabel(filename, path) {
  var attachment_icon = {
    png: "fa-file-image-o",
  ...
  return (
    ...
    '"></i> ' +
    filename +
    '</a> <a class="download-attachment" href="' +
    path +
    '" download title="Download"> <i class="fa fa-arrow-circle-down text-blue"></i></a>'
  );
}
...
function getRelativeTime(time) {
  ...
  if (!time) return;
  if (typeof time == "number") time = String(time);
 ...
 * @param {function} cb
 */
export function showNotification(type, message, options, cb) {
...
export function fileUpload(event, path, cb) {
