...
class BaseMySQL_model extends MY_Model
{
  ...
  public function generateDatatable($select, $columns, $join = array(), $columnJoin = array(),  $as = null)
  {
    if ($select == null) $select = "$this->table.*";
    $data = $this->db->select($select)->select("$as");
    foreach ($join as $i => $j) {
      $data->join($j, "$this->table.$columnJoin[$i]=$j.id");
    }
    $data = $data->get($this->table)->result_array();
